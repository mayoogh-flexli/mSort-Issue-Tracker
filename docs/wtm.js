const WTM = {
    ...
    async load() {
        this.showLoading('Loading WTM issues...');
        ...
        try {
            ...
            const response = await fetch(
                `${window.MSORT_CONFIG.GITHUB_API_BASE}/repos/${window.MSORT_CONFIG.REPO_OWNER}/${window.MSORT_CONFIG.REPO_NAME}/issues?labels=wtm-issue&state=open`,
                { headers }
            );
    ...
    render(issues) {
        if (issues.length === 0) {
            document.getElementById('content').innerHTML = `
                <div class="no-issues">
                    <h2>All WTM Systems Operational</h2>
                    <p>No open WTM issues found</p>
                </div>
            `;
            return;
        }
            ...
            html += `
                <div class="bot-card ${statusClass}">
                    ...
                    <div class="issue-info">
                        <div class="issue-row">
                            <span class="issue-label">Zone:</span>
                            <span class="issue-value">${issue.zone}</span>
                        </div>
                        <div class="issue-row">
                            <span class="issue-label">Column:</span>
                            <span class="issue-value">${issue.column}</span>
                        </div>
                        <div class="issue-row">
    ...
    showError(message) {
        document.getElementById('content').innerHTML = `
            <div class="error">
                ...
                <br><br>
                <button class="btn" onclick="WTM.load()">Try Again</button>
            </div>
        `;
    },
...
// Initialize WTM dashboard when DOM is ready
